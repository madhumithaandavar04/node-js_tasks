...
import * as buddyService from "../services/buddyServices.js";
/**
 * get all buddies
 * @param {*} req 
 * @param {*} res 
...
export const updateBuddy = async (req, res, next) => {
    const updateQuery = { employeeId: req.params.id };
    const body = req.body;
    try {
        const updatedBuddy = await buddyService.update(updateQuery, body);
        logger.debug(`update the buddy ${employeeId}`);
...
export const deleteBuddy = async (req, res, next) => {
    try {
        ...
        logger.debug(`delete the buddy ${buddy.realName}`);
        res.status(201).json({
            status: "success",
